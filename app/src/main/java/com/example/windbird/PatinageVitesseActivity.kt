// PatinageVitesseActivity.kt — COPIE EXACTE du Biathlon fonctionnel
...
class PatinageVitesseActivity : Activity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        statusText = TextView(this).apply {
            text = "⛸️ PATINAGE VITESSE - Joueur: ${tournamentData.playerNames[currentPlayerIndex]} | Distance: 0m"
    ...
    private fun updateStatus() {
        statusText.text = when (gameState) {
            GameState.SKIING -> "⛸️ PATINAGE VITESSE - ${tournamentData.playerNames[currentPlayerIndex]} | Distance: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.SHOOTING -> "🎯 PATINAGE VITESSE - ${tournamentData.playerNames[currentPlayerIndex]} | Tir ${shotsFired}/5 — Touchés: $targetsHit"
            GameState.FINAL_SKIING -> "⛸️ PATINAGE VITESSE - ${tournamentData.playerNames[currentPlayerIndex]} | Sprint final: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.FINISHED -> "✅ PATINAGE VITESSE - ${tournamentData.playerNames[currentPlayerIndex]} | Score final: ${calculateScore()} points"
        }
    }
    ...
    inner class PatinageVitesseView(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if (gameState == GameState.SHOOTING || gameState == GameState.FINISHED) {
                ...
                canvas.drawText("⛸️ ZONE PATINAGE VITESSE ⛸️", w/2f, 60f, paint)
