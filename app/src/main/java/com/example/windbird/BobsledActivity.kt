// BobsledActivity.kt — COPIE EXACTE du Biathlon fonctionnel
...
class BobsledActivity : Activity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        statusText = TextView(this).apply {
            text = "🛷 BOBSLEIGH - Joueur: ${tournamentData.playerNames[currentPlayerIndex]} | Distance: 0m"
    ...
    private fun updateStatus() {
        statusText.text = when (gameState) {
            GameState.SKIING -> "🛷 BOBSLEIGH - ${tournamentData.playerNames[currentPlayerIndex]} | Distance: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.SHOOTING -> "🎯 BOBSLEIGH - ${tournamentData.playerNames[currentPlayerIndex]} | Tir ${shotsFired}/5 — Touchés: $targetsHit"
            GameState.FINAL_SKIING -> "🛷 BOBSLEIGH - ${tournamentData.playerNames[currentPlayerIndex]} | Sprint final: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.FINISHED -> "✅ BOBSLEIGH - ${tournamentData.playerNames[currentPlayerIndex]} | Score final: ${calculateScore()} points"
        }
    }
    ...
    inner class BobsledView(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if (gameState == GameState.SHOOTING || gameState == GameState.FINISHED) {
                ...
                canvas.drawText("🛷 ZONE BOBSLEIGH 🛷", w/2f, 60f, paint)
