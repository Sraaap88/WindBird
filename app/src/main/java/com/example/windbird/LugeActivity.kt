// LugeActivity.kt — COPIE EXACTE du Biathlon fonctionnel
...
class LugeActivity : Activity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        statusText = TextView(this).apply {
            text = "🛷 LUGE - Joueur: ${tournamentData.playerNames[currentPlayerIndex]} | Distance: 0m"
    ...
    private fun updateStatus() {
        statusText.text = when (gameState) {
            GameState.SKIING -> "🛷 LUGE - ${tournamentData.playerNames[currentPlayerIndex]} | Distance: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.SHOOTING -> "🎯 LUGE - ${tournamentData.playerNames[currentPlayerIndex]} | Tir ${shotsFired}/5 — Touchés: $targetsHit"
            GameState.FINAL_SKIING -> "🛷 LUGE - ${tournamentData.playerNames[currentPlayerIndex]} | Sprint final: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.FINISHED -> "✅ LUGE - ${tournamentData.playerNames[currentPlayerIndex]} | Score final: ${calculateScore()} points"
        }
    }
    ...
    inner class LugeView(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if (gameState == GameState.SHOOTING || gameState == GameState.FINISHED) {
                ...
                canvas.drawText("🛷 ZONE LUGE 🛷", w/2f, 60f, paint)
