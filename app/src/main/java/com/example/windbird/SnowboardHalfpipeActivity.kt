// SnowboardHalfpipeActivity.kt — COPIE EXACTE du Biathlon fonctionnel
...
class SnowboardHalfpipeActivity : Activity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        statusText = TextView(this).apply {
            text = "🏂 SNOWBOARD HALFPIPE - Joueur: ${tournamentData.playerNames[currentPlayerIndex]} | Distance: 0m"
    ...
    private fun updateStatus() {
        statusText.text = when (gameState) {
            GameState.SKIING -> "🏂 SNOWBOARD HALFPIPE - ${tournamentData.playerNames[currentPlayerIndex]} | Distance: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.SHOOTING -> "🎯 SNOWBOARD HALFPIPE - ${tournamentData.playerNames[currentPlayerIndex]} | Tir ${shotsFired}/5 — Touchés: $targetsHit"
            GameState.FINAL_SKIING -> "🏂 SNOWBOARD HALFPIPE - ${tournamentData.playerNames[currentPlayerIndex]} | Sprint final: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.FINISHED -> "✅ SNOWBOARD HALFPIPE - ${tournamentData.playerNames[currentPlayerIndex]} | Score final: ${calculateScore()} points"
        }
    }
    ...
    inner class SnowboardHalfpipeView(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if (gameState == GameState.SHOOTING || gameState == GameState.FINISHED) {
                ...
                canvas.drawText("🏂 ZONE SNOWBOARD HALFPIPE 🏂", w/2f, 60f, paint)
