// SkiFreestyleActivity.kt — COPIE EXACTE du Biathlon fonctionnel
...
class SkiFreestyleActivity : Activity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        statusText = TextView(this).apply {
            text = "🎿 SKI FREESTYLE - Joueur: ${tournamentData.playerNames[currentPlayerIndex]} | Distance: 0m"
    ...
    private fun updateStatus() {
        statusText.text = when (gameState) {
            GameState.SKIING -> "🎿 SKI FREESTYLE - ${tournamentData.playerNames[currentPlayerIndex]} | Distance: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.SHOOTING -> "🎯 SKI FREESTYLE - ${tournamentData.playerNames[currentPlayerIndex]} | Tir ${shotsFired}/5 — Touchés: $targetsHit"
            GameState.FINAL_SKIING -> "🎿 SKI FREESTYLE - ${tournamentData.playerNames[currentPlayerIndex]} | Sprint final: ${distance.toInt()}m / ${totalDistance.toInt()}m"
            GameState.FINISHED -> "✅ SKI FREESTYLE - ${tournamentData.playerNames[currentPlayerIndex]} | Score final: ${calculateScore()} points"
        }
    }
    ...
    inner class SkiFreestyleView(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if (gameState == GameState.SHOOTING || gameState == GameState.FINISHED) {
                ...
                canvas.drawText("🎿 ZONE SKI FREESTYLE 🎿", w/2f, 60f, paint)
